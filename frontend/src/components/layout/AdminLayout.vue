...
const sidebarCollapsedKey = 'acadocs:admin-sidebar-collapsed'
...
onMounted(async () => {
  ...
  try {
    unreadCount.value = await notificationService.adminUnreadCount()
  } catch { /* non-critical */ }
})
onBeforeUnmount(() => {
...
const navItems = [
  ...
  { to: '/admin/registrars',      label: 'Registrars',       icon: 'M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0z' },
  { to: '/admin/requests',        label: 'Requests',         icon: 'M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z' },
  { to: '/admin/calendar',        label: 'Calendar',         icon: 'M8 7V3m8 4V3M5 11h14M6 5h12a2 2 0 012 2v12a2 2 0 01-2 2H6a2 2 0 01-2-2V7a2 2 0 012-2z' },
  { to: '/admin/queue',           label: 'Queue',            icon: 'M4 6h16M4 10h16M4 14h16M4 18h16' },
  { to: '/admin/payment-methods', label: 'Payment Methods',  icon: 'M17 9V7a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2m2 4h10a2 2 0 002-2v-6a2 2 0 00-2-2H9a2 2 0 00-2 2v6a2 2 0 002 2zm7-5a2 2 0 11-4 0 2 2 0 014 0z' },
  { to: '/admin/api-keys',        label: 'API Keys',         icon: 'M15 7a2 2 0 012 2m4 0a6 6 0 01-7.743 5.743L11 17H9v2H7v2H4a1 1 0 01-1-1v-2.586a1 1 0 01.293-.707l5.964-5.964A6 6 0 1121 9z' },
  ...
  { to: '/admin/request-analytics', label: 'Request Analytics', icon: 'M3 3v18h18M7 13l3-3 3 2 4-5' },
  { to: '/admin/document-types',  label: 'Document Types',   icon: 'M19 11H5m14 0a2 2 0 012 2v6a2 2 0 01-2 2H5a2 2 0 01-2-2v-6a2 2 0 012-2m14 0V9a2 2 0 00-2-2M5 11V9a2 2 0 012-2m0 0V5a2 2 0 012-2h6a2 2 0 012 2v2M7 7h10' },
  ...
  <div class="h-screen overflow-hidden bg-slate-50">
    ...
    >
      ...
      >
        ...
        <div :class="['leading-tight', sidebarCollapsed ? 'lg:hidden' : '']">
          ...
          <span class="block text-sky-200 text-xs font-normal">Admin Panel</span>
        </div>
      </div>
      ...
      <div class="px-3 py-4 border-t border-white/10 space-y-1">
        <router-link
          to="/admin/profile"
          :class="[
            'flex items-center rounded-lg px-3 py-2 transition hover:bg-white/10',
            sidebarCollapsed ? 'lg:justify-center lg:gap-0' : 'gap-2.5'
          ]"
          :title="sidebarCollapsed ? auth.user?.email : undefined"
        >
          <div class="h-7 w-7 rounded-full bg-sky-300/30 flex items-center justify-center shrink-0">
            <svg class="h-4 w-4 text-sky-100" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.75">
              <path stroke-linecap="round" stroke-linejoin="round" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" />
            </svg>
          </div>
          <span :class="['text-xs text-sky-100 truncate', sidebarCollapsed ? 'lg:hidden' : '']">{{ auth.user?.email }}</span>
        </router-link>
        <button
          @click="logout"
          :class="[
    ...
    <div :class="['flex h-screen min-w-0 flex-col transition-all duration-200', sidebarCollapsed ? 'lg:ml-20' : 'lg:ml-64']">
      <header class="h-16 shrink-0 bg-white border-b border-gray-200 flex items-center px-4 lg:px-6 gap-3">
        ...
        <div class="h-1.5 w-6 rounded-full hidden lg:block" style="background: linear-gradient(90deg, #0ea5e9, #2563eb);"></div>
        <h1 class="text-sm font-semibold text-gray-700">Admin Panel</h1>
        <div ref="notificationPopoverRef" class="relative ml-auto">
          <button
            type="button"
            class="relative inline-flex items-center justify-center rounded-lg p-2 text-gray-500 transition hover:bg-gray-100"
          ...
          >
            ...
            <span
              v-if="unreadCount > 0"
              class="absolute top-1 right-1 flex h-4 min-w-4 items-center justify-center rounded-full bg-red-500 px-0.5 text-[10px] font-bold text-white"
          ...
          >
            ...
            <ul v-else class="max-h-80 overflow-y-auto divide-y divide-gray-100">
              <li v-for="n in previewNotifications" :key="n.id" :class="['flex gap-3 px-4 py-3 transition hover:bg-gray-50', !n.is_read ? 'bg-sky-50/60' : '']">
            ...
            <div class="grid grid-cols-2 border-t border-gray-100 bg-gray-50">
              ...
              <button type="button" class="flex items-center justify-center gap-2 px-3 py-3 text-sm font-medium text-gray-700 transition hover:bg-gray-100 disabled:cursor-not-allowed disabled:text-gray-400" :disabled="unreadCount === 0" @click="markEveryNotificationRead">
                <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.75">
                  <path stroke-linecap="round" stroke-linejoin="round" d="M5 13l4 4L19 7" />
                </svg>
                Mark all read
