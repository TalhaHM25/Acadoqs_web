...
// Registrars are locked to their own level
const myLevel = auth.isCollegeRegistrar ? 'college' : 'senior_high'

const form = reactive({
  category_id:        0,
  name:               '',
  description:        '',
  level:              myLevel as string,
  base_fee:           0,
...
async function load() {
  ...
  try {
    ...
    types.value      = (typesRes as any[]).filter((t: any) => t.level === myLevel || t.level === 'all')
...
function openCreate() {
  ...
  Object.assign(form, { category_id: categories.value[0]?.id ?? 0, name: '', description: '', level: myLevel, base_fee: 0, certified_copy_fee: 0, processing_days: 3 })
  formErrors.value = {}
...
function openEdit(type: any) {
  ...
  Object.assign(form, {
    category_id: type.category_id ?? categories.value[0]?.id ?? 0,
    name: type.name, description: type.description ?? '',
    level: type.level ?? myLevel,
    base_fee: type.base_fee, certified_copy_fee: type.certified_copy_fee,
...
async function toggle(type: any) {
...
<template>
    <div class="space-y-5">
      ...
      <div v-else class="bg-white rounded-xl border border-gray-200 overflow-hidden">
        <div v-if="!types.length" class="py-10 text-center text-sm text-gray-400">No document types yet.</div>
        <table v-else class="min-w-full divide-y divide-gray-100 text-sm">
          <thead class="bg-slate-50">
            <tr>
              ...
              <th class="px-5 py-3 text-right text-xs font-semibold text-gray-500 uppercase">Base Fee</th>
              <th class="px-5 py-3 text-right text-xs font-semibold text-gray-500 uppercase">Cert. Fee</th>
              <th class="px-5 py-3 text-center text-xs font-semibold text-gray-500 uppercase">Days</th>
              <th class="px-5 py-3 text-center text-xs font-semibold text-gray-500 uppercase">Active</th>
              <th class="px-5 py-3" />
            </tr>
          </thead>
          <tbody class="divide-y divide-gray-100">
            <tr v-for="type in types" :key="type.id" class="hover:bg-slate-50">
              ...
              <td class="px-5 py-3 text-xs text-gray-500">{{ type.category_name }}</td>
        ...
        <div class="bg-white rounded-2xl shadow-xl w-full max-w-md p-6 space-y-5">
          ...
          <div class="space-y-4">
            ...
            <div class="grid grid-cols-2 gap-3">
              ...
              <FormField label="Certified Copy Fee (+₱)">
                <input v-model.number="form.certified_copy_fee" type="number" min="0" step="0.01"
                  class="block w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:border-sky-500 focus:outline-none focus:ring-1 focus:ring-sky-500" />
              </FormField>
            </div>
            <FormField label="Processing Days" required>
          ...
          <div class="flex gap-3 pt-1">
            ...
            <AppButton :full="true" :loading="saving" @click="save">{{ editTarget ? 'Save Changes' : 'Create' }}</AppButton>
          </div>
        </div>
      </div>
    </div>
