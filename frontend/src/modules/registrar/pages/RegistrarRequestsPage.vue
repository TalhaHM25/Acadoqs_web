...
const rows = ref<DocumentRequest[]>([])
...
function queryRequestId(): number | null {
  const value = Number(queryValue('request_id'))
  return Number.isInteger(value) && value > 0 ? value : null
}

function hydrateFiltersFromQuery() {
  const queryStatus = queryValue('status')
  status.value = statusOptions.some(option => option.value === queryStatus) ? queryStatus : ''
  const querySource = queryValue('source')
  source.value = sourceOptions.some(option => option.value === querySource) ? querySource : ''
  dateFrom.value = queryValue('date_from')
  dateTo.value = queryValue('date_to')
}

async function load() {
  loading.value = true
  try {
    const res = await adminService.listRequests({
...
function statusLabel(requestStatus: RequestStatus) {
  return STATUS_LABELS[requestStatus] ?? requestStatus
}

function isSavingStatusAction(requestId: number, requestStatus: RequestStatus) {
  return savingStatusAction.value?.requestId === requestId && savingStatusAction.value.status === requestStatus
}
...
watch([status, source, dateFrom, dateTo], syncQueryAndLoad)
watch(page, load)
</script>
...
<template>
  <div class="space-y-5">
    <div class="flex flex-wrap items-center justify-between gap-3">
      <h1 class="text-xl font-semibold text-gray-900">Document Requests</h1>
    ...
    <div class="rounded-xl border border-gray-100 bg-white p-4 shadow-sm">
      <div class="grid gap-3 md:grid-cols-[minmax(0,1fr)_repeat(4,minmax(8rem,auto))_auto]">
        ...
        />
        <select v-model="status" class="rounded-lg border border-gray-300 px-3 py-2 text-sm focus:border-sky-500 focus:outline-none focus:ring-1 focus:ring-sky-500">
          <option v-for="o in statusOptions" :key="o.value" :value="o.value">{{ o.label }}</option>
        </select>
        <select v-model="source" class="rounded-lg border border-gray-300 px-3 py-2 text-sm focus:border-sky-500 focus:outline-none focus:ring-1 focus:ring-sky-500">
    ...
    <div v-else class="overflow-hidden rounded-xl border border-gray-200 bg-white">
      <div v-if="!rows.length" class="py-10 text-center text-sm text-gray-400">No requests found.</div>
      <div v-else class="overflow-x-auto">
        <table class="w-full min-w-[1080px] divide-y divide-gray-100 text-sm">
          <thead class="bg-slate-50">
            <tr>
              <th class="w-40 px-5 py-3 text-left text-xs font-semibold uppercase text-gray-500">Request #</th>
              <th class="w-64 px-5 py-3 text-left text-xs font-semibold uppercase text-gray-500">Student</th>
              <th class="px-5 py-3 text-left text-xs font-semibold uppercase text-gray-500">Source</th>
              <th class="w-40 px-5 py-3 text-left text-xs font-semibold uppercase text-gray-500">Status</th>
              <th class="w-48 px-5 py-3 text-left text-xs font-semibold uppercase text-gray-500">Due Reminder</th>
              <th class="w-32 px-5 py-3 text-right text-xs font-semibold uppercase text-gray-500">Fee</th>
              <th class="w-36 px-5 py-3 text-right text-xs font-semibold uppercase text-gray-500">Date</th>
              <th class="w-64 px-5 py-3 text-right text-xs font-semibold uppercase text-gray-500">Actions</th>
            </tr>
          </thead>
          <tbody class="divide-y divide-gray-100">
            <tr v-for="req in rows" :key="req.id" class="hover:bg-slate-50">
              <td class="w-40 px-5 py-3 whitespace-nowrap">
                <span class="font-mono text-xs text-gray-600">{{ req.request_number }}</span>
              </td>
              <td class="w-64 px-5 py-3 text-gray-800">
                <p class="max-w-56 truncate whitespace-nowrap font-medium text-gray-900">{{ req.req_first_name }} {{ req.req_last_name }}</p>
                <p class="max-w-56 truncate whitespace-nowrap text-xs text-gray-400">{{ req.document_type_name }}</p>
              </td>
              <td class="px-5 py-3"><SourceBadge :source="req.request_source" /></td>
              <td class="w-40 px-5 py-3 whitespace-nowrap">
                <span :class="['inline-flex shrink-0 rounded-full px-2.5 py-0.5 text-xs font-medium whitespace-nowrap', STATUS_COLORS[req.status]]">
                  {{ statusLabel(req.status) }}
                </span>
              </td>
              <td class="w-48 px-5 py-3 whitespace-nowrap">
                <div class="inline-flex min-w-max shrink-0 whitespace-nowrap">
                  <DueDateReminder :due-date="req.expected_release_at" :status="req.status" />
                </div>
              </td>
              <td class="w-32 px-5 py-3 text-right whitespace-nowrap text-gray-700">{{ formatCurrency(req.total_fee) }}</td>
              <td class="w-36 px-5 py-3 text-right whitespace-nowrap text-xs text-gray-500">{{ formatDate(req.created_at) }}</td>
              <td class="w-64 px-5 py-3 whitespace-nowrap">
                <div class="flex min-w-max justify-end gap-2 whitespace-nowrap">
                  ...
                  <button
                    type="button"
                    class="inline-flex shrink-0 items-center gap-1 rounded-lg bg-emerald-600 px-3 py-1.5 text-xs font-semibold text-white transition hover:bg-emerald-700 whitespace-nowrap"
