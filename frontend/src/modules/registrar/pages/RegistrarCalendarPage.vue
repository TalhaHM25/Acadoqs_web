...
<template>
  <div class="space-y-5">
    <div class="flex flex-wrap items-center justify-between gap-3">
      <div>
        <p class="text-sm text-gray-500">Registrar calendar</p>
        ...
        <section class="min-w-0 overflow-hidden rounded-xl border border-gray-200 bg-white">
          ...
          <div class="grid grid-cols-7">
            <button
              ...
              type="button"
              :class="[
                'min-h-28 border-b border-r border-gray-100 p-2 text-left transition hover:bg-emerald-50',
                !cell.inMonth ? 'bg-slate-50 text-gray-300' : 'bg-white text-gray-700',
                selectedDates.includes(cell.key) ? 'bg-emerald-50 ring-2 ring-inset ring-emerald-500' : ''
              ]"
            ...
            >
              ...
              <div class="mt-2 space-y-1">
                <div
                  ...
                  :key="request.id"
                  class="truncate rounded bg-emerald-100 px-2 py-1 text-[11px] font-medium text-emerald-800 hover:bg-emerald-200"
                  @click.stop="openRequestDetails(request.id)"
                >
                  {{ request.request_number }}
                </div>
                <p v-if="(requestsByDate.get(cell.key) ?? []).length > 3" class="text-[11px] font-semibold text-emerald-700">
            ...
            <div v-for="group in selectedGroups" :key="group.key" class="rounded-xl border border-gray-200 bg-white p-4">
              ...
              <div v-if="group.requests.length" class="mt-3 space-y-2">
                <router-link
                  ...
                  :key="request.id"
                  :to="{ name: 'RegistrarRequests', query: { request_id: String(request.id) } }"
